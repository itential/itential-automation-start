...
class Authentication {
  ...
  getIndex(authId) {
    let indx;
    const userIndex = this.users.some((users, index) => {
      indx = index;
      return users.authId === authId;
    });
    if (userIndex) return indx;
    return -1;
  }

  checkTokenValidity(indx) {
    if (this.users[indx].expires_in === -1) {
      console.log('Static Token Valid');
      return true;
    }
    // if dynamic token
  ...
  async getNewPasswToken(indx, callback) {
    try {
      console.log('No token, Loging in using password');
      const url = `${this.users[indx].hostname}/login`;
      const data = {
        user: {
          username: this.users[indx].username,
          password: this.users[indx].password
        }
      };
  ...
  async renewPaswToken(indx, callback) {
    // oauth or username /pass
    try {
      console.log('Renewing password token');
      const url = `${this.users[indx].hostname}/login`;// *****can test url error here ***** */
      const data = {
        user: {
          username: this.users[indx].username,
          password: this.users[indx].password
  ...
  async getNewOAuthToken(indx, callback) {
    try {
      // oauth or username /pass
      console.log('Getting new OAuth Token');
      const url = `${this.users[indx].hostname}/oauth/token`;
      const payload = {
        client_id: this.users[indx].client_id,
        client_secret: this.users[indx].client_secret,
        grant_type: this.users[indx].grant_type
      };
      const data = external_querystring_namespaceObject.stringify(payload);
      const config = {
        headers: {
  ...
  async renewOAuthToken(indx, callback) {
    try {
      // oauth or username /pass
      console.log('Renewing the OAuth token');
      const url = `${this.users[indx].hostname}/oauth/token`;
      const payload = {
        client_id: this.users[indx].client_id,
        client_secret: this.users[indx].client_secret,
    ...
    try {
      ...
      if (this.users[indx].token.length !== 0) {
        ...
        if (this.users[indx].authType.oAuth) {
          await this.renewOAuthToken(indx, (res, err) => {
            if (err) {
              return callback(null, err);
            }
            return callback(res);
          ...
          // else password user
          await this.renewPaswToken(indx, (res, err) => {
            if (err) {
              return callback(null, err);
            }
            return callback(res);
      ...
      if(!Object.prototype.hasOwnProperty.call(checkUser, 'validity') || checkUser.validity === false){
        ...
        if (valid) {
          ...
          if (user.authType.oAuth) {
            ...
            user.token = '';
            user.authId = `${user.hostname}_${user.username}`;
            user.expires_in = null;
...
class GenericAPI {
  ...
  /**
   * Perform a generic API request to IAP
   *
   * @param {String} method - HTTP method to perform
   * @param {String} hyperSchema - hyperSchema to use
   * @param {String} href - Specific href to use
   * @param {function callback(success, fail)} - a function that returns a successful response or error message
   * @param {JSON} data - A JSON object to use in POST, PUT or DELETE requests
   * @returns
   */
...
class JSONFormsAPI {
  ...
  /**
   * Get the schema for a JSON form given a form id
   *
   * @param {String} formId - the id of the form you wish to retrieve
   * @param {function callback(success, fail)} - a function that returns an object or error
   * @returns an object that represents a JSON form
   */
...
class OperationsManagerAPI {
  ...
  */
  ...
  async cancelAutomations(ids, callback) {
    try {
      ...
      if (err.response.status === 500 && err.response.data.message === 'Failed to cancel all jobs') {
        let message = '';
        if (Object.keys(err.response.data.data).length === 1) {
          message = `Failed to cancel ${Object.keys(err.response.data.data).toString()} as it was already canceled, complete, or didn't exist`;
        } else {
          message = `Failed to cancel ${Object.keys(err.response.data.data).toString()} as they were already canceled, complete, or don't exist`;
        }
  ...
  /**
   * Gets a list of automations and their associated triggers.
   *
   * @param {function callback(automations, fail)} - a fumction that returns a list automations or a failure message
   * @returns - automations array or failure message
   */
  async listAutomations(callback) {
    try {
      let automations = [];
      let triggers = [];
      let token = '';
      ...
      // Loop through automations and create an empty array of triggers at each item. Also, populate the map.
      for (let x = 0; x < automations.length; x++) {
        automations[x].triggers = [];
        const automationId = automations[x]._id;
        automationMap.set(automationId, x);
      }
      // Now, loop through triggers and insert trigger at matching automation using the map
      for (let y = 0; y < triggers.length; y++) {
        // Grab automation Id for trigger
        const automationTriggerId = triggers[y].actionId;
        // Get corresponding automation array index using map
        const automationIndex = automationMap.get(automationTriggerId);
        if (automationIndex !== undefined) {
          automations[automationIndex].triggers.push(triggers[y]);
        }
      }
  ...
  /**
   * Start an automation using a trigger
   *
   * @param {String} automationName -name of the automation you wish to run
   * @param {String} triggerName -name of the trigger attached to the automation you wish to run
  ...
  async startAutomation(automationName, triggerName, callback, formData = {}) {
    try {
      ...
      for (let x = 0; x < automationsWithTriggers.length; x++) {
        if (automationsWithTriggers[x].name === automationName) {
          const { triggers } = automationsWithTriggers[x];
          for (let y = 0; y < triggers.length; y++) {
...
class TransformationsAPI {
  ...
  /**
   * 
   * @param {*} id - transformation id to run
   * @param {*} incoming - incoming transformation data
   * @param {function callback(success, fail)} - a function that returns a successfull transformation or an error
   * @returns A response that outlines the result of the transformation
   */
...
async function run() {
  ...
  const automation_status = (0,core.getInput)("automation_status");
          ...
          opsManager.startAutomationEndpoint(api_endpoint, api_endpoint_body, (res, err)=> {
            ...
            if(err){
              if(typeof err === "string"){
                ...
                (0,core.setFailed)(err.response.data);
              } else (0,core.setFailed)(`Failed while triggering automation: Please check the instance configuration and credentials
                An Itential account is required to get credentials needed to configure the Github Actions.
                In order to utilize this action, you would need to have an active \`Itential Automation Platform\` (IAP).
                If you are an existing customer, please contact your Itential account team for additional details.
                For new customers interested in an Itential trial, please click [here](https://www.itential.com/get-started/) to request one.`);
